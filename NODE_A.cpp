...
void *serverTimer (void *data) {
	...
	// Stop and Wait
	if(params[0] == 1) {
	    if(td->ss.sendQ[td->ss.LFQ].msg[HLEN] != 0 &&  !td->ss.sendQ[td->ss.LFQ].ack &&
	    (td->ss.sendQ[td->ss.LFQ].start.tv_sec != 0 || td->ss.sendQ[td->ss.LFQ].start.tv_usec != 0)) {
		printf("LFQ: %d\n", td->ss.LFQ);
            	// Check timeout 
            	gettimeofday(&tmp, NULL);
            	// Timeout occurs here 
            	if (timeDiff(td->ss.sendQ[td->ss.LFQ].start, tmp) > params[2]) {
            	    pthread_mutex_lock(&td->send);
